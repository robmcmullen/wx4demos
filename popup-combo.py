...
class TestTransientPopup(wx.PopupTransientWindow):
    ...
    def __init__(self, parent, style, log):
        ...
        sampleList = ['zero', 'one', 'two', 'three', 'four', 'five',
                      'six', 'seven', 'eight']
        cb = wx.ComboBox(panel, 500, "default value", (90, 50),
                         (160, -1), sampleList,
                         wx.CB_DROPDOWN)
...
class TestPopupWithListbox(wx.PopupWindow):
    def __init__(self, parent, style, log):
        ...
        self.log = log
        panel = wx.Panel(self)
        panel.SetBackgroundColour("#B6C1FF")
        ...
        self.lb.Bind(wx.EVT_KEY_DOWN, self.on_key_down)
        self.lb.Bind(wx.EVT_KEY_UP, self.on_key_up)
    ...
    def on_key_down(self, evt):
        ...
        self.log.debug("key down: %s, selection=%s" % (key, index))
        evt.Skip()

    def on_key_up(self, evt):
        key = evt.GetKeyCode()
        index = self.GetSelection()
        self.log.debug("key up: %s, selection=%s" % (key, index))
        evt.Skip()
